      ...
      <section className="rounded-3xl border border-red-100 bg-white/95 p-5 shadow-[0_12px_40px_rgba(220,38,38,0.08)] sm:p-7">
        <div className="flex items-start justify-between gap-3">
          ...
          <Link
            ...
            href={branchCode === MAIN_BRANCH_CODE ? "/" : `/branches/${branchCode}`}
          >
            🏠 홈으로
          </Link>
        </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-sm">
          {branches.map((branch) => (
            <Link
              ...
              href={branch.code === MAIN_BRANCH_CODE ? "/admin" : `/admin/${branch.code}`}
            >
              {branch.name}
            </Link>
          ))}
        ...
        <div className="mt-5 grid grid-cols-1 gap-2 sm:grid-cols-3 sm:gap-3">
          <div className="rounded-2xl border border-red-100 bg-white/95 px-3 py-3 text-sm shadow-sm sm:text-base">이번달 운행 건수: <b>{totals._count}</b> 회</div>
          <div className="rounded-2xl border border-red-100 bg-white/95 px-3 py-3 text-sm shadow-sm sm:text-base">이번달 주행 합계: <b>{formatNumber(totals._sum.distance)}</b> km</div>
          <div className="rounded-2xl border border-red-100 bg-white/95 px-3 py-3 text-sm shadow-sm sm:text-base">이번달 통행료 합계: <b>{formatNumber(totals._sum.tollCost)}</b> 원</div>
        </div>
            ...
            <tbody>
              {byVehicle.map(({ v, agg, latest }) => (
                <tr key={v.id} className="border-b align-top last:border-b-0">
                  ...
                  <td className="p-2 text-right whitespace-nowrap">{formatNumber(agg._count)}</td>
                  <td className="p-2 text-right whitespace-nowrap">{formatNumber(agg._sum.distance)}</td>
                  <td className="p-2 text-right whitespace-nowrap">{formatNumber(agg._sum.tollCost)}</td>
                  <td className="p-2 text-right whitespace-nowrap">{formatNumber(latest?.evRemainPct)}%</td>
                  <td className="p-2 text-right whitespace-nowrap">{formatNumber(latest?.hipassBalance)}</td>
                  <td className="p-2 text-right whitespace-nowrap">{formatNumber(latest?.odoEnd)}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody>
              {recentTrips.map((t) => (
                <tr key={t.id} className="border-b align-top last:border-b-0">
                  ...
                  <td className="p-2 text-right whitespace-nowrap">{formatNumber(t.tollCost)}</td>
                  <td className="p-2 text-right whitespace-nowrap">{formatNumber(t.evRemainPct)}%</td>
                  <td className="p-2 text-right whitespace-nowrap">{formatNumber(t.hipassBalance)}</td>
