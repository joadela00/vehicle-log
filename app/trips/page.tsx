...
const getVehiclesAll = unstable_cache(
  () =>
...
export default async function TripsPage({
  ...
  const branchNamePromise = branchCode
    ? prisma.vehicle
        .findFirst({ where: { branchCode }, select: { branchName: true } })
        .then((v) => v?.branchName || branchCode)
    : Promise.resolve("");
        ...
        {trips.length === 0 ? (
          ...
          <>
            ...
            <div className="mt-5 grid gap-3 sm:hidden">
              {trips.map((t) => (
                <article key={t.id} className="rounded-2xl border border-red-100 bg-white p-4 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <div className="text-lg font-bold tracking-tight">{t.date.toISOString().slice(0, 10)}</div>
